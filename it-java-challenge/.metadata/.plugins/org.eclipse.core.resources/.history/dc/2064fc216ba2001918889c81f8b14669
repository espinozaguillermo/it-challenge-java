...
public class InscripcionesCursoController {
	...
	public List<InscripcionesCurso> getAllInscripcionesCursosActualesByAlumno(
			@PathVariable(value = "id") Integer alumnoId) {
		if (alumnoId != null) {
	...
	public List<Object> getEstadoInscripcionesCursosByAlumno(@PathVariable(value = "id") Integer alumnoId) {
	...
	public List<Object> getPromedioGralInscripcionesCursosByAlumno(@PathVariable(value = "id") Integer alumnoId) throws ResourceNotFoundException {
		List<Object> list = new ArrayList<Object>();
		if (alumnoId != null) {
			...
			for (InscripcionesCarrera item : listinscripcionescarrera) {
				...
				Carrera carrera = carreraRepository.findById(item.getCarrera().getId())
						.orElseThrow(() -> new ResourceNotFoundException("Carrera no encontrado id :: " + item.getCarrera().getId()));
				List<Object> tmplist = new ArrayList<Object>();
				...
				for (Curso curso : listcurso) {
					...
					for (InscripcionesCurso inscripcioncurso : listinscripcionescurso) {
						if (inscripcioncurso.getCalificacion() != null && inscripcioncurso.getCalificacion() >= curso.getCalificacionaprobada()) {
	...
	public ResponseEntity<InscripcionesCurso> getInscripcionesCursoById(
			@PathVariable(value = "id") Integer inscripcionescursoId) throws ResourceNotFoundException {
		InscripcionesCurso inscripcionescurso = inscripcionescursoRepository.findById(inscripcionescursoId).orElseThrow(
				() -> new ResourceNotFoundException("InscripcionesCurso no encontrada id :: " + inscripcionescursoId));
	...
	public ResponseEntity<InscripcionesCurso> updateInscripcionesCurso(
			@PathVariable(value = "id") Integer inscripcionescursoId,
